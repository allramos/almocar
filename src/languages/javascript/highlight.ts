...
const KEYWORDS = new Set([
  ...
  'switch', 'case', 'default',
  'function', 'class', 'new', 'delete', 'typeof', 'instanceof',
...
function tokenizeJavaScript(source: string): Tok[][] {
...
function tokenizeLine(line: string, inBlockComment: boolean): [Tok[], boolean] {
  ...
  while (i < line.length) {
    ...
    if (/[0-9]/.test(c)) {
      let j = i;
      while (j < line.length && /[0-9.xXa-fA-FeEnN]/.test(line[j])) j++;
    ...
    if (/[A-Za-z_$]/.test(c)) {
      ...
      else if (word === 'null' || word === 'undefined') kind = 'kw';
...
export const javascriptHighlight: HighlightConfig = {
  keywords: KEYWORDS,
  types: TYPES,
