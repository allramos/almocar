...
export default function App() {
  ...
  const [theme, setTheme] = useState<Theme>(() => (localStorage.getItem('almocar.theme') as Theme) ?? 'dark');
  ...
  useEffect(() => {
    ...
    playRef.current = window.setTimeout(() => setStepIndex(i => Math.min(i + 1, steps.length - 1)), speed);
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden relative" style={{ zIndex: 0 }}>
      <Header
      ...
      <main className="flex-1 grid gap-3 px-6 pb-3 grid-cols-12 min-h-0">
        ...
        <section className="col-span-12 lg:col-span-4 panel flex flex-col min-h-0">
          <div className="panel-title">
            ...
            <span className="meta">{mode === 'editing' ? 'edição' : 'execução'}</span>
          </div>
        ...
        <section className="col-span-12 lg:col-span-3 flex flex-col gap-3 min-h-0">
          ...
          <div className="flex-[1.6] min-h-0">
            <TraceLog steps={visSteps} current={visStepIndex} onSelect={setStepIndex} />
          </div>
        </section>
      </main>
...
function Header({
  exampleKey, onExample, onCozinhar, onEditar, mode,
  theme, onToggleTheme, language, onLanguageChange, onAbout,
}: {
  exampleKey: string;
      ...
      >
        <div className="flex items-baseline justify-between mb-4">
          <h2
            ...
            style={{ fontSize: '32px', fontVariationSettings: "'opsz' 144", letterSpacing: '-0.02em' }}
          >
            sobre o <span className="text-ember">almoçar</span>
          </h2>
          <button onClick={onClose} className="btn btn-icon" aria-label="Fechar">✕</button>
        </div>

        <div className="text-ink-dim text-[13px] leading-relaxed space-y-4 font-sans">
          <p>
            <span className="text-ink font-medium">ALMOÇAR</span> é um acrônimo para{' '}
            <span className="text-ink">Ambiente</span> <span className="text-ink">Lúdico</span> <span className="text-ink">de</span> <span className="text-ink">Monitoramento</span> <span className="text-ink">de</span> <span className="text-ink">Operações</span>
            <span className="text-ink">e</span> <span className="text-ink">Código</span> <span className="text-ink">Algorítmico</span> <span className="text-ink">Responsivo</span>.
          </p>
          <p>
            A ferramenta permite visualizar a execução de algoritmos passo a passo,
            acompanhando o estado da memória, das variáveis e da saída a cada
            instrução, com o objetivo de auxiliar estudantes a desenvolver um
            modelo mental claro do funcionamento do código.
          </p>
          <div className="hairline my-2" />
          <p className="text-ink-mute text-xs leading-relaxed">
            Desenvolvido por <span className="text-ink">Prof. Allan Jheyson</span><br />
            Curso Técnico em Informática · <span className="text-ink">Colégio Técnico de Bom Jesus</span><br />
